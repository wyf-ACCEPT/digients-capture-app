...
class _SuccessScreenState extends State<SuccessScreen>
  ...
  void initState() {
    ...
    _copyTimer = Timer.periodic(const Duration(milliseconds: 2400), (_) {
      if (!mounted) return;
      setState(() => _copyIndex = (_copyIndex + 1) % 3);
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    final copy = [
      l10n.successCopyUploading,
      l10n.successCopyKeepConnection,
      l10n.successCopyPointsSoon,
    ];
    return Scaffold(
                ...
                children: [
                  ScaleTransition(
                    scale: CurvedAnimation(
                        parent: _checkCtl, curve: Curves.elasticOut),
                    child: Container(
                      width: 120,
                      height: 120,
...
class _ConfettiState extends State<_Confetti>
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      animation: _ctl,
      builder: (_, __) =>
          CustomPaint(painter: _ConfettiPainter(_particles, _ctl.value)),
    );
  }
}
  ...
  void paint(Canvas canvas, Size size) {
    for (final p in particles) {
      ...
      final rect = Rect.fromCenter(
          center: Offset(p.x * size.width, y), width: p.size, height: p.size);
      canvas.drawRRect(
